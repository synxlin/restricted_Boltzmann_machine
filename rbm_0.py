...
class ReadData:
    ...
    def read(self, Path, trainflag=0, mnistflag=0, type=1, level=0): # 0 784，答案为dim维度  1 784+dim 答案为数字   -1  分类，答案为数字
        """
        读取数据函数
        :param Path: 数据所在路径
...
if __name__ == '__main__':
    ...
    dim = 10
    layers = 3
    num_hidden = [784, 500, 500, 2000, 10]
    learning_rate = 0.1
    pieces = 100
    max_epochs_1 = 50
    max_epochs_2 = 200
    ...
    figtrainPath = 'D:\\Download\\学习\\模式识别\\大作业\\data\\train\\Process\\'
    ...
    r = RBM_net(layers=layers, dim=num_hidden, learning_rate=learning_rate, Path=path, mode=0)

    r.train_rbms(batch_data=batch_data, batch_label=batch_label, max_epochs_1=max_epochs_1, max_epochs_2=max_epochs_2, test_set=test_set, test_label_set=test_label_set, test_name_set=test_name_set)
