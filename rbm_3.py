...
class RBM_net:
    ...
    def train_rbms(self, batch_data, batch_label=None, max_epochs_1=50, max_epochs_2=200, test_set=None,
            ...
            for epoch in range(0, max_epochs_2):
                ...
                if not (test_set == None):
                    ...
                    for i in range(0, num_teset_set):
                        tmp_result = r.predict(batch_test=test_set[i], batch_test_label=test_label_set[i],
    ...
    def predict(self, batch_test, batch_test_label, test_name):
        ...
        if self.mode == 1:
            ...
            for i in range(0, self.layers):
                if i == self.layers - 1:
                    test_data = list(
                        map(lambda y: np.array(list(map(lambda x: x + [0] * self.dim[self.layers + 1], y))), test_data))
...
if __name__ == '__main__':
    ...
    directory = ['D:\\Download\\学习\\模式识别\\大作业\\data\\test\\Process2\\0\\',
    ...
    rd = ReadData(dim=dim)
    set = rd.read(Path=mnistPath, trainflag=1, mnistflag=1, type=1)
    train_data = set[0]
    batch_data = rd.makebatches(train_data, pieces=pieces)
    batch_data = batch_data[0]

    set = rd.read(Path=mnistPath, trainflag=0, mnistflag=1, type=1)
    test_data = [np.array(set[0])]
    testans = set[1]

    test_set = [test_data]
    test_label_set = [testans]
    test_name_set = ['MNIST_test']

    for file in range(0, 5):
        figPath = directory[file]
        for level in range(0, 6):
            if file == 0 and level > 0:
                break
            set = rd.read(Path=figPath, trainflag=0, mnistflag=0, type=1, level=level)
            if len(set[0]) > 0:
                test_data_core = [np.array(set[0])]
                testans_core = set[1]
                test_set.append(test_data_core)
                test_label_set.append(testans_core)
                test_name = 'Test' + ('_%d' % file) + ('_%d' % level)
    ...
    with open(datafile, 'wt') as fp:
        fp.write('\n method:%s\n visible num:%s\n hidden num:%s\n learning rate:%s\n piece size:%s\n epochs:%s\n' % (
            method, num_visible, num_hidden, learning_rate, pieces, max_epochs))
    ...
    for epoch in range(0, max_epochs):
        if path:
            ...
            with open(datafile, 'at') as fp:
                fp.write('\n epoch: %d \n' %epoch)
        r.train(batch_data, max_epochs=1)
        print(r.weights)
        print("Train RBM Successfully")
