...
class ReadData:
    ...
    def read(self, Path, trainflag=0, mnistflag=0, type=1, reverseflag=1, level=0): # 0 784，答案为dim维度  1 784+dim 答案为数字   -1  分类，答案为数字
        """
        读取数据函数
        :param Path: 数据所在路径
        ...
        if mnistflag:
            ...
            if trainflag:
                ...
                for item in range(0, len(dataset[0])):
                    ...
                    item_hd = self.pro(item_hd, maxflag=1, reverseflag=0)
                    if type > 0:
            ...
            for f in fig:
                if int(f[2]) == level:
                    ...
                    item_hd = self.pro(im, maxflag=1, reverseflag=reverseflag)
                    if type > 0:
                        for i in range(0, self.dim):
                            item_hd.append(0)
                        set.append(item_hd)
    ...
    def pro(self, item, maxflag, reverseflag):
        if maxflag:
            m = max(item)
        else:
            m = 255
        if reverseflag:
            result = list(map(lambda x: 1 - x / m, item))
        else:
