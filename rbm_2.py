...
class RBM_net:
    ...
    def predict(self, batch_test, batch_test_label, test_name):
        ...
        if self.mode == 1:
            ...
            for i in range(0, self.layers):
                if i == self.layers - 1:
                    test_data = list(map(lambda y: np.array(list(map(lambda x: x+[0]*self.dim[self.layers+1], y))), test_data))
...
if __name__ == '__main__':
    ...
    r = RBM_each(num_visible=num_visible, num_hidden=num_hidden, learning_rate=learning_rate, Path=path)

    #r.train(train_data, pieces=pieces, max_epochs=max_epochs)
